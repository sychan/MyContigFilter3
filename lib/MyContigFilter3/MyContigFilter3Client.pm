package MyContigFilter3::MyContigFilter3Client;
...
MyContigFilter3::MyContigFilter3Client
...
sub new
{
    ...
    my $self = {
	client => MyContigFilter3::MyContigFilter3Client::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a MyContigFilter3.FilterContigsParams
$output is a MyContigFilter3.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	assembly_input_ref has a value which is a MyContigFilter3.assembly_ref
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	assembly_output has a value which is a MyContigFilter3.assembly_ref
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "MyContigFilter3.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "MyContigFilter3.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for MyContigFilter3::MyContigFilter3Client\n";
    }
    if ($sMajor == 0) {
        warn "MyContigFilter3::MyContigFilter3Client version is $svr_version. API subject to change.\n";
    }
}
...
package MyContigFilter3::MyContigFilter3Client::RpcClient;
